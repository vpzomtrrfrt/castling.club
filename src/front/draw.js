...
const { ASSETS_BASE } = require("../util/consts");
const { words } = require("../util/misc");
...
module.exports = async ({ cache, hmacSecret, origin, router }) => {
  ...
  const draw = async slug => {
    const { pieces, side, move } = slugToArray(slug);
  ...
  const imageUrlFor = (pieces, side, move = null) => {
